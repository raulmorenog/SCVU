...
realPole = []; realwn = []; realamort = [];
imagPole = []; imagwn = []; imagamort = [];
for i = 1:length(Poles)
    if (imag(Poles(i)) == 0)
        realPole = [realPole; Poles(i)];
        realwn = [realwn; wn(i)];
        realamort = [realamort; amort(i)];
    else
        imagPole = [imagPole; Poles(i)];
        imagwn = [imagwn; wn(i)];
        imagamort = [imagamort; amort(i)];
    end
end
FT_lat.Poles = Poles;
...
FT_lat.dutchroll.wn = imagwn(1);
FT_lat.dutchroll.period = 2*pi/FT_lat.dutchroll.wn;
FT_lat.dutchroll.amort = imagamort(1);
